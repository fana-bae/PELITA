...
export async function createHabit(formData) {
  ...
  // ── Validasi input ────────────────────────────────────────
  let name, type, category, icon, color, description
  try {
  ...
  const { data, error } = await supabase
    .from('habits')
    .insert({ user_id: user.id, name, type, category, icon, color, description })
    .select()
    .single()
...
export async function updateHabit(habitId, formData) {
  ...
  let name, category, icon, color, description
  try {
    name        = sanitizeText(formData.name,        { maxLength: 100, fieldName: 'Nama habit' })
    category    = assertEnum(formData.category || 'other', ALLOWED.HABIT_CATEGORY, 'Kategori')
    icon        = sanitizeText(formData.icon || '⭐', { maxLength: 10, fieldName: 'Ikon', required: false }) || '⭐'
  ...
  const { data, error } = await supabase
    .from('habits')
    .update({ name, category, icon, color, description })
    .eq('id', habitId)
...
// ============================================================
export async function toggleHabitLog(habitId, habitType) {
  const supabase = await createClient()
  const user = await getAuthUser(supabase) // ← tidak ada lagi console.log!
